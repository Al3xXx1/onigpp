...
- `oniguruma`: Oniguruma のネイティブ構文とセマンティクスを使用。後方参照、独占的量指定子、アトミックグループなど Oniguruma 固有の機能が有効になります。**警告**: 後方参照を有効にすると、特定のパターンで指数関数的なバックトラッキングが発生する可能性があります。パフォーマンスが重要なコードでは、独占的量指定子やアトミックグループの使用を検討してください。
